<?php
    ...
    kickice();
    makeshift();
    $ur = mysqli_fetch_assoc(mysqli_query($conn,'SELECT * FROM radcheck WHERE username="'.$_SESSION['uid'].'"'));
    ...
    if(isset($_POST['save'])){
        if(!empty($_FILES['pic'])){
            if(is_uploaded_file($_FILES['pic']['tmp_name'])){
                ...
                $pic = $target_file;
                $lisa = 'UPDATE radcheck SET fullname="'.$_POST['fullname'].'",value="'.$_POST['values'].'",pic="'.$pic.'" WHERE username="'.$_SESSION['uid'].'"';
        }else{
                $lisa = 'UPDATE radcheck SET fullname="'.$_POST['fullname'].'",value="'.$_POST['values'].'" WHERE username="'.$_SESSION['uid'].'"';
        }
        if(mysqli_query($conn,$lisa)==true){
